...
print("==="*20)
print("file name: greedy_vllm_dev.py")
...
llm = LLM(model=repo, dtype=torch.bfloat16, tensor_parallel_size=n_gpu, enforce_eager=True, gpu_memory_utilization=0.95, trust_remote_code=True) # tensor_parralel size는 내 상황에 맞게 세팅가능
...
for i in range(len(outputs)):
    ...
    output_logprobs = [elements[0].logprobs[num][output_tokens[num]].logprob for num in range(len(output_tokens))]
